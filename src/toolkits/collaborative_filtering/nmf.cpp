    ...
    void update(graphchi_vertex<VertexDataType, EdgeDataType> &vertex, graphchi_context &gcontext) {
        ...
        if (gcontext.iteration == 0) {
            if (vertex.num_outedges() == 0 && vertex.id() < M)
                Rcpp::Rcerr<<"NMF algorithm can not work when the row " << vertex.id() << " of the matrix contains all zeros" << std::endl;
            for(int e=0; e < vertex.num_edges(); e++) {
                float observation = vertex.edge(e)->get_data();
                if (observation < 0 ) {
                    Rcpp::Rcerr<<"Found a negative entry in matirx row " << vertex.id() << " with value: " << observation << std::endl;
                }
            }
            return;
        }
        ...
        for(int e=0; e < vertex.num_edges(); e++) {
            ...
            if (prediction == 0)
                Rcpp::Rcerr<<"Got into numerical error! Please submit a bug report." << std::endl;
...
void output_nmf_result(std::string filename) {
    ...
    Rcpp::Rcout << "NMF output files (in matrix market format): " << filename << "_U.mm" <<
