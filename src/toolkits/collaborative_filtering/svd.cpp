...
void init_lanczos(bipartite_graph_descriptor & info) {
    ...
    Rcpp::Rcout<<"Allocated a total of: " << ((double)actual_vector_len * info.total() * sizeof(double)/ 1e6) << " MB for storing vectors." << std::endl;
}
...
void output_svd_result(std::string filename) {
    ...
    Rcpp::Rcout << "SVD output files (in matrix market format): " << filename << "_U.mm" <<
...
vec lanczos( bipartite_graph_descriptor & info, timer & mytimer, vec & errest,
    ...
    if (save_vectors) {
        Rcpp::Rcout<<"Going to save output vectors U and V" << std::endl;
        if (nconv == 0)
            Rcpp::Rcerr<<"No converged vectors. Aborting the save operation" << std::endl;
...
int main(int argc,  const char *argv[]) {
    ...
    if (nv < nsv) {
        Rcpp::Rcerr<<"Please set the number of vectors --nv=XX, to be at least the number of support vectors --nsv=XX or larger" << std::endl;
    }
    ...
    Rcpp::Rcout << "Load matrix " << training << std::endl;
    ...
    else Rcpp::Rcerr<<"--tokens_per_row=XX should be either 2 or 3 input columns" << std::endl;
    ...
    Rcpp::Rcout << "Lanczos finished " << mytimer.current_time() << std::endl;
