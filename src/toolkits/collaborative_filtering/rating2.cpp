...
float svdpp_predict(const vertex_data& user, const vertex_data& movie, const float rating, double & prediction, void * extra = NULL) {
    ...
    if (std::isnan(err))
        Rcpp::Rcerr<<"Got into numerical errors. Try to decrease step size using the command line: svdpp_user_bias_step, svdpp_item_bias_step, svdpp_user_factor2_step, svdpp_user_factor_step, svdpp_item_step" << std::endl;
    return err*err;
}
...
float biassgd_predict(const vertex_data& user,
    ...
    float err = rating - prediction;
    if (std::isnan(err))
        Rcpp::Rcerr<<"Got into numerical errors. Try to decrease step size using bias-SGD command line arugments)" << std::endl;
    return err*err;
...
float rbm_predict(const rbm_user & usr,
    ...
    for(int r = 0; r < rbm_bins; ++r) {
        ...
        //if (std::isinf(zz))
        //  Rcpp::Rcout<<" mov.bi[r] " << mov.bi[r] << " dot: " << dot(usr.h, &mov.w[r*D]) << std::endl;
        ret += zz * (float)(r);
...
void output_knn_result(std::string filename) {
    ...
    Rcpp::Rcout << "Rating output files (in matrix market format): " << filename << ".ratings" <<
...
int main(int argc, const char ** argv) {
    ...
    if (knn_sample_percent <= 0 || knn_sample_percent > 1)
        Rcpp::Rcerr<<"Sample percente should be in the range (0, 1] " << std::endl;

    num_ratings   = get_option_int("num_ratings", 10);
    if (num_ratings <= 0)
        Rcpp::Rcerr<<"num_ratings, the number of recomended items for each user, should be >=1 " << std::endl;
    ...
    if (algorithm == "svdpp" || algorithm == "svd++")
        ...
        algo = RBM;
    else Rcpp::Rcerr<<"--algorithm should be svd++ or biassgd or rbm"<<std::endl;
    ...
    else Rcpp::Rcerr<<"--tokens_per_row should be either 3 or 4" << std::endl;
    ...
    if ((uint)num_ratings > N) {
        Rcpp::Rcout<<"num_ratings is too big - setting it to: " << N << std::endl;
        num_ratings = N;
    }
    srand(time(NULL));
    ...
    if (users_without_ratings > 0)
        Rcpp::Rcout<<"Found " << users_without_ratings << " without ratings. For those users no items are recommended (item id 0)" << std::endl;

    if (users_no_ratings > 0)
        Rcpp::Rcout<<"Failed to compute ratings for " << users_no_ratings << " Users. For those users no items are recommended (item id 0)" << std::endl;
