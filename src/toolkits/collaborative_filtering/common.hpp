...
#include "util.hpp"
#include "graphchi/graphchi_basic_includes.hpp"
#include "graphchi/api/vertex_aggregator.hpp"
#include "graphchi/preprocessing/sharder.hpp"
#include "matrix_factorization/matrixmarket/mmio.h"
#include "matrix_factorization/matrixmarket/mmio.c"
...
void parse_command_line_args() {
    ...
    if (minval >= maxval)
        Rcpp::Rcout<<"Min allowed rating (--minval) should be smaller than max allowed rating (--maxval)" << std::endl;
        
    valrange      = maxval - minval;
    assert(valrange > 0);
    quiet    = get_option_int("quiet", 1);
    ...
    if (loss == "square")
        ...
        loss_type = AP;
    else Rcpp::Rcout<<"Loss type should be one of [square,logistic,abs] (for example, --loss==square);" << std::endl;
    ...
    if (kfold_cross_validation_index > 0) {
        if (kfold_cross_validation_index >= kfold_cross_validation)
            Rcpp::Rcout<<"kfold_cross_validation index should be between 0 to kfold_cross_validation-1 parameter" << std::endl;
    }
    
    if (kfold_cross_validation != 0) {
        Rcpp::Rcout << "Activating kfold cross vlidation with K=" << kfold_cross_validation << std::endl;
        
        if (validation != "" || test != "")
			Rcpp::stop("Using cross validation, validation file (--validation) and test file (--test) should be empty\n");
			
        //removing tmp file (if present)
        int rc = system(("rm -fR " + training + "_kfold_tmp_file").c_str());
        if (rc != 0)
            Rcpp::Rcout<<"Failed to delete temp file. Please check permissions." << std::endl;
        ...
        if (rc != 0)
            Rcpp::Rcout<<"Failed to link temp file. Please check permissions." << std::endl;
...
void print_config() {
    Rcpp::Rcout << "[feature_width] => [" << D << "]" << std::endl;
    Rcpp::Rcout << "[users] => [" << M << "]" << std::endl;
    Rcpp::Rcout << "[movies] => [" << N << "]" <<std::endl;
    Rcpp::Rcout << "[training_ratings] => [" << L << "]" << std::endl;
    Rcpp::Rcout << "[number_of_threads] => [" << number_of_omp_threads() << "]" <<std::endl;
    Rcpp::Rcout << "[membudget_Mb] => [" << get_option_int("membudget_mb") << "]" <<std::endl;
}
