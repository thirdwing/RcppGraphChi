...
void sample_alpha(double res2) {
    ...
    if (nuAlpha > 0) {
        ...
        if (debug)
            Rcpp::Rcout<<"Sampling from alpha" <<nuAlpha_<<" "<<iWalpha<<" "<< iiWalpha_<<" "<<alpha<<endl;
...
mat calc_MMT(int start_pos, int end_pos, vec &Umean) {
    ...
    for (int i=start_pos; i< end_pos; i++) {
        ...
        if (debug && (i==start_pos || i == end_pos-1))
            Rcpp::Rcout<<" clmn "<<i<< " vec: " << mean <<std::endl;
...
void sample_U() {
    ...
    vec dMu = - Umean;
    if (debug)
        Rcpp::Rcout<<"dMu:"<<dMu<<"beta0: "<<beta0[0]<<" beta0_ "<<beta0_<<" nu0_ " <<nu0_<<" mu0_ " << mu0_<<endl;
    ...
    if (debug)
        Rcpp::Rcout<<iW0<<UUT<<UmeanT<<dMuT<<W0_<<tmp<<nu0_<<endl;
    ...
    if (debug)
        Rcpp::Rcout<<"Sampling from U" <<A_U<<" "<<mu_U<<" "<<Umean<<" "<<W0_<<tmp<<endl;
}
...
void sample_V() {
    ...
    vec dMu = - Vmean;
    if (debug)
        Rcpp::Rcout<<"dMu:"<<dMu<<"beta0: "<<beta0[0]<<" beta0_ "<<beta0_<<" nu0_ " <<nu0_<<endl;
    ...
    mat tmp = (W0_+transpose(W0_))*0.5;
    if (debug)
        Rcpp::Rcout<<"iW0: "<<iW0<<" VVT: "<<VVT<<" VmeanT: "<<VmeanT<<" dMuT: " <<dMuT<<"W0_"<< W0_<<" tmp: " << tmp<<" nu0_: "<<nu0_<<endl;
    ...
    if (debug)
        Rcpp::Rcout<<"Sampling from V: A_V" <<A_V<<" mu_V: "<<mu_V<<" Vmean: "<<Vmean<<" W0_: "<<W0_<<" tmp: "<<tmp<<endl;
}
...
void output_pmf_result(std::string filename) {
    ...
    Rcpp::Rcout << "PMF output files (in matrix market format): " << filename << "_U.mm" <<
