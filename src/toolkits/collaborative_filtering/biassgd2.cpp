        ...
        if ( vertex.num_outedges() > 0) {
            ...
            for(int e=0; e < vertex.num_edges(); e++) {
                ...
                if (std::isnan(err) || std::isinf(err))
                    Rcpp::Rcerr<<"BIASSGD got into numerical error. Please tune step size using --biassgd_gamma and biassgd_lambda" << std::endl;
...
void output_biassgd_result(std::string filename) {
    ...
    Rcpp::Rcout << "SVDPP output files (in matrix market format): " << filename << "_U.mm" <<
...
int main(int argc, const char ** argv) {
    ...
    if (maxval == 1e100 || minval == -1e100)
        Rcpp::Rcerr<<"You must set min allowed rating and max allowed rating using the --minval and --maval flags" << std::endl;
