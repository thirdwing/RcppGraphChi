...
void validation_rmse_N(
    ...
    Rcpp::Rcout<<"avg validation prediction: " << avg_pred/(double)nz << std::endl;
    assert(Le > 0);
    ...
    if (skipped_features > 0)
        Rcpp::Rcout<<"Skipped " << skipped_features << " when reading from file. " << std::endl;
    if (skipped_nodes > 0)
        Rcpp::Rcout<<"Skipped " << skipped_nodes << " when reading from file. " << std::endl;
}
...
void training_rmse_N(int iteration, graphchi_context &gcontext, bool items = false) {
    ...
    if (calc_error)
        Rcpp::Rcout<< std::setw(10) << mytimer.current_time() << ") Iteration: " << std::setw(3) <<iteration<<" Training " << error_names[loss_type] << " : " <<std::setw(10)<< dtraining_rmse << " Train err: " << std::setw(10) << (total_errors/(double)L);
    else
