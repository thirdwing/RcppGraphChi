...
void init_lanczos(bipartite_graph_descriptor & info) {
    ...
    Rcpp::Rcout<<"Allocated a total of: " <<
...
vec one_sided_lanczos( bipartite_graph_descriptor & info, timer & mytimer, vec & errest,
    ...
    if (save_vectors) {
        Rcpp::Rcout<<"Going to save output vectors V" << std::endl;
        if (nconv == 0)
...
int main(int argc,  const char *argv[]) {
    ...
    if (nv < nsv) {
        Rcpp::Rcerr<<"Please set the number of vectors --nv=XX, to be greater than the number of support vectors --nsv=XX " << std::endl;
    }
    ...
    Rcpp::Rcout << "Lanczos finished in " << mytimer.current_time() << std::endl;
