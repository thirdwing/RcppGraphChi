    ...
    void update(graphchi_vertex<VertexDataType, EdgeDataType> &vertex, graphchi_context &gcontext) {
        ...
        if (gcontext.iteration == 0) {
            ...
            if (is_user(vertex.id()) && vertex.num_outedges() == 0)
                Rcpp::Rcout<<"Vertex: " << vertex.id() << " with no edges: " << std::endl;
            return;
        }
...
void output_libfm_result(std::string filename) {
    ...
    Rcpp::Rcout << " LIBFM output files (in matrix market format): " << filename << "_U.mm" << ", " << filename + "_V.mm " << filename + "_T.mm, " << filename << "_L.mm, " << filename <<  "_global_mean.mm, " << filename << "_U_bias.mm " << filename << "_V_bias.mm, " << filename << "_T_bias.mm, " << filename << "_L_bias.mm " <<std::endl;
}
