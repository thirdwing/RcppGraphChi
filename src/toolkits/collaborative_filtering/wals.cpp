...
void output_als_result(std::string filename) {
    ...
    Rcpp::Rcout << "WALS output files (in matrix market format): " << filename << "_U.mm" <<
...
int main(int argc, const char ** argv) {
    ...
    if (unittest == 1) {
        if (dtraining_rmse > 0.03)
            Rcpp::Rcerr<<"Unit test 1 failed. Training RMSE is: " << training_rmse << std::endl;
        if (dvalidation_rmse > 0.61)
            Rcpp::Rcerr<<"Unit test 1 failed. Validation RMSE is: " << validation_rmse << std::endl;
