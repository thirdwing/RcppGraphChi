...
void get_offsets(std::vector<int> & offsets) {
    ...
    if (fc.hash_strings) {
        for (uint j=2; j< offsets.size()-1; j++) {
            ...
            Rcpp::Rcerr<<"Offset " << j+1 << " is: " << offsets[j+1] << std::endl;
        }
    } else {
        assert(false);
    }
...
bool is_user(vid_t id) {
    return id < M;
}
bool is_item(vid_t id) {
    return id >= M && id < M+N;
}
bool is_time(vid_t id) {
    return id >= M+N;
}
...
struct edge_data {
    ...
    edge_data() {
        weight = 0;
        memset(features, 0, sizeof(float)*FEATURE_WIDTH);
    }

    edge_data(float weight, float * valarray, int size): weight(weight) {
        memcpy(features, valarray, sizeof(float)*size);
    }
};
...
float compute_prediction(
    ...
    // Rcpp::Rcerr<<"BUG: can not compute prediction for new user and new item" << std::endl;
    ...
    if (I != (uint)-1) {
        ...
        if (debug2) Rcpp::Rcout<<"I: "<<I<< " offset: " << I+fc.offsets[loc] << " " << head(latent_factors_inmem[fc.offsets[loc]].pvec,5).transpose() << " " << latent_factors_inmem[fc.offsets[loc]].bias << std::endl ;
        if (node_array[index]->pvec[0] >= 1e5)
            Rcpp::Rcerr<<"Got into numerical problem, try to decrease SGD step size" << std::endl;
    ...
    if (J != (uint)-1) {
        ...
        if (debug2) Rcpp::Rcout<<"J: "<<J<< " offset: " << J+fc.offsets[loc] << " " << head(latent_factors_inmem[J+fc.offsets[loc]].pvec,5).transpose() <<" " << latent_factors_inmem[J+fc.offsets[loc]].bias << std::endl ;
        if (node_array[index]->pvec[0] >= 1e5)
            Rcpp::Rcerr<<"Got into numerical problem, try to decrease SGD step size" << std::endl;
    } else node_array[index] = &vertex_dummy;
    index++;
    ...
    for (int j=0; j< fc.total_features; j++) {
        ...
        if (fc.real_features_indicators[fc.feature_positions[j]]) {
            ...
            if (pos < 0 || pos >= latent_factors_inmem.size())
                Rcpp::Rcerr<<"Bug: j is: " << j << " fc.total_features " << fc.total_features << " index : " << index << " loc: " << loc <<
            ...
            if (debug2) Rcpp::Rcout<<"j+index: "<<j+index<< " offset: " << pos << " " << head(latent_factors_inmem[pos].pvec,5).transpose() << " " << latent_factors_inmem[pos].bias << std::endl;
        }
            ...
            FOR_ITERATOR(j, latent_factors_inmem[I+fc.offsets[0]].features) {
                ...
                if (user_links != "") {
                    ...
                    if (j.index() >= fc.node_id_maps[pos2].string2nodeid.size()) {
                        Rcpp::Rcout<<"Bug : " << j.index()  << " is big: " << loc << " " << fc.node_id_maps[pos2].string2nodeid.size() <<std::endl;
                        //assert(j.index() < (int)fc.node_id_maps[loc].string2nodeid.size());
                    }
                    assert(pos >= 0 && pos < (int)latent_factors_inmem.size());
                }
                //Rcpp::Rcout<<"setting index " << i+index << " to: " << pos << std::endl;
...
void read_node_features(std::string base_filename, bool square, feature_control & fc, bool user, bool binary) {
    ...
    while(true) {
        ...
        if (pch == NULL)
            Rcpp::Rcerr<<"Error reading line " << lines << " [ " << linebuf_debug << " ] " << std::endl;
        ...
        while (pch != NULL) {
            ...
            if (binary) {
                ...
                if (J >= fc.node_id_maps[2+fc.total_features+fc.node_features-1].string2nodeid.size()) {
                    Rcpp::Rcout<<"BUG_1: I " << I << " J: " << J << " PCH: " << pch << " " << fc.node_id_maps[2+fc.total_features+fc.node_features-1].string2nodeid.size() << " " << fc.node_features << " " <<fc.total_features <<std::endl;
                    assert(false);
                }
            } else {
                pch = strtok(NULL, ptokens);
                if (pch == NULL)
                    Rcpp::Rcerr<<"Failed to read feture value" << std::endl;
                val = atof(pch);
            }
    ...
    assert(tokens > 0);
    Rcpp::Rcerr<<"Read a total of " << lines << " node features. Tokens: " << tokens << " avg tokens: " << (lines/tokens)
...
void read_node_links(std::string base_filename, bool square, feature_control & fc, bool user, bool binary) {
    ...
    while(true) {
        ...
        pch = strtok(NULL, ptokens);
        if (pch == NULL)
            Rcpp::Rcerr<<"Failed to read to field [ " << linebuf_debug << " ] " << std::endl;
    ...
    Rcpp::Rcerr<<"Read a total of " << lines << " node features. Tokens: " << tokens << " user? " << user <<  " new entries: " << fc.node_id_maps[user? 0 : 1].string2nodeid.size() << std::endl;
}
...
void validation_rmse_N(std::string & filename,
    ...
    if ((filename == "") || !file_exists(filename)) {
        if (validation != "")
            Rcpp::Rcout << "Validation file was specified, but not found:" << validation << std::endl;
        Rcpp::Rcout<<std::endl;
        return;
    }
    FILE *f = NULL;
    ...
    if (f == NULL) {
        Rcpp::Rcout<<"Failed to open validation data. Skipping."<<std::endl;
        return;
    }

    if ((M > 0 && N > 0) && (Me != M || Ne != N))
        Rcpp::Rcout<<"Input size of validation matrix must be identical to training matrix, namely " << M << "x" << N << std::endl;
    ...
    for (size_t i=0; i<nz; i++) {
        ...
        if (!read_line(f, validation, i, I, J, val, valarray, VALIDATION, linebuf_debug))
            Rcpp::Rcerr<<"Failed to read line: " << i << " in file: " <<filename << std::endl;
        ...
        if (active_edge) {
            ...
            for (int k=0; k< calc_feature_node_array_size(I,J); k++)
                node_array[k] = NULL;
            vec sum;
            //Rcpp::Rcout<<"Going to compute validation for : " <<I << " " <<J << " " << val << " " << size << " " << sum << " " << calc_feature_node_array_size(I,J) << " " << Le << std::endl;
            compute_prediction(I, J, val, prediction, &valarray[0], size, prediction_func, &sum, node_array, VALIDATION);
            //Rcpp::Rcout<<"Computed prediction is: " << prediction << std::endl;
            delete [] node_array;
    ...
    Rcpp::Rcout<< (type == TRAINING ? "  Training RMSE: " :"  Validation RMSE: ") << std::setw(10) << dvalidation_rmse;
    if (!calc_error)
        Rcpp::Rcout << std::endl;
    else Rcpp::Rcout << (type == TRAINING? " Training Error" : " Validation error: ") << std::setw(10) << validation_error/howmany << std::endl;
...
void test_predictions_N(
    ...
    if (test == "") {
        Rcpp::Rcout<<"No test file was found, skipping test predictions " << std::endl;
        return;
    }

    if (!file_exists(test)) {
        if (test != (training + "t"))
            Rcpp::Rcout<<" test predictions file was specified but not found: " << test << std::endl;
        return;
    }

    detect_matrix_size(test, f, Mt, Nt, nz);
    if (f == NULL) {
        Rcpp::Rcout<<"Failed to open test file. Skipping " << std::endl;
        return;
    }
    ...
    for (i=0; i<nz; i++) {
        ...
        if (!read_line(f, test, i, I, J, val, valarray, TEST, linebuf_debug))
            Rcpp::Rcerr<<"Failed to read line: " <<i << " in file: " << test << std::endl;
    ...
    if (i != nz)
        Rcpp::Rcerr<<"Missing input lines in test file. Should be : " << nz << " found only " << i << std::endl;
    fclose(f);
    fclose(fout);
...
float gensgd_predict(const vertex_data** node_array, int node_array_size,
    ...
    *sum = zeros(D);
    ...
    if (debug2)
        Rcpp::Rcout<<"Adding global mean: "<< globalMean << std::endl;
    ...
    for (int i=0; i< node_array_size; i++) {
        if (i >= 2 && fc.real_features_indicators[fc.feature_positions[i-2]])
            ...
            if (debug2)
                Rcpp::Rcout<<"Adding bias: "<< i << " : " << node_array[i]->bias << std::endl;
        }
    }
    ...
    for (int j=0; j< D; j++) {
        for (int i=0; i< node_array_size; i++) {
            ...
            if (debug2)
                if (sum->operator[](j) >= 1e5)
                    Rcpp::Rcerr<<"Got into numerical problems. Try to decrease step size" << std::endl;
            if (i >= 2 && fc.real_features_indicators[fc.feature_positions[i-2]])
        ...
        if (debug2 && j < 3)
            Rcpp::Rcout<<" Adding sum: " << sum_sqr[j] << std::endl;
...
void init_gensgd(bool load_factors_from_file) {
    ...
    Rcpp::Rcerr<<"Going to calculate: " << howmany << " offsets." << std::endl;
...
void training_rmse_N(int iteration, graphchi_context &gcontext, bool items = false) {
    ...
    if (calc_error)
        Rcpp::Rcout<< std::setw(10) << mytimer.current_time() << ") Iteration: " << std::setw(3) <<iteration<<" Training RMSE: " << std::setw(10)<< dtraining_rmse << " Train err: " << std::setw(10) << (total_errors/(double)L);
    else
        Rcpp::Rcout<< std::setw(10) << mytimer.current_time() << ") Iteration: " << std::setw(3) <<iteration<<" Training RMSE: " << std::setw(10)<< dtraining_rmse;
}
...
struct GensgdVerticesInMemProgram : public GraphChiProgram<VertexDataType, EdgeDataType> {
    ...
    void print_step_size() {
        Rcpp::Rcout<<"Step size 1 " << sum(stat1)/(double)L <<
    ...
    void after_iteration(int iteration, graphchi_context &gcontext) {
        if (iteration == 1 && vertex_with_no_edges > 0)
            Rcpp::Rcout<<"There are " << vertex_with_no_edges << " users without ratings" << std::endl;
...
void output_model(std::string filename) {
    ...
    Rcpp::Rcout << "GENSGD output files (in matrix market format): " << filename << "_U.mm" << "," << filename << "_global_mean.mm" << std::endl;
...
int main(int argc, const char ** argv) {
    ...
    if (D <=2 || D>= 300)
        Rcpp::Rcerr<<"Allowed range for latent factor vector D is [2,300]." << std::endl;
    ...
    if (has_header_titles && header_titles.size() == 0 && ! validation_only)
        Rcpp::Rcerr<<"Please delete temp files (using --clean_cache=1 ) and run again" << std::endl;
    ...
    if (fc.node_features) {
        ...
        Rcpp::Rcerr<<"Going to add " << toadd << std::endl;
        vertex_data data;
    ...
    Rcpp::Rcout <<"Total selected features: " << fc.total_features << " : " << std::endl;
    for (int i=0; i < MAX_FEATURES+3; i++)
        if (fc.feature_selection[i])
            Rcpp::Rcout<<"Selected feature: " << std::setw(3) << i << " : " << (has_header_titles? header_titles[i] : "") <<std::endl;
    Rcpp::Rcout<<"Target variable " << std::setw(3) << fc.val_pos << " : " << (has_header_titles? header_titles[fc.val_pos] : "") <<std::endl;
    Rcpp::Rcout<<"From            " << std::setw(3) << fc.from_pos<< " : " << (has_header_titles? header_titles[fc.from_pos] : "") <<std::endl;
    Rcpp::Rcout<<"To              " << std::setw(3) << fc.to_pos  << " : " << (has_header_titles? header_titles[fc.to_pos] : "") <<std::endl;
    ...
    if (new_validation_users > 0)
        Rcpp::Rcout<<"Found " << new_validation_users<< " new users with no information about them in training dataset!" << std::endl;
    ...
    if (new_test_users > 0)
        Rcpp::Rcout<<"Found " << new_test_users<< " new test users with no information about them in training dataset!" << std::endl;
