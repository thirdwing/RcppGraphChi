        ...
        if ( vertex.num_outedges() > 0) {
            ...
            for(int e=0; e < vertex.num_edges(); e++) {
                ...
                if (std::isnan(err) || std::isinf(err))
                    Rcpp::Rcerr<<"SGD got into numerical error. Please tune step size using --sgd_gamma and sgd_lambda" << std::endl;
...
void output_sgd_result(std::string filename) {
    ...
    Rcpp::Rcout << "SGD output files (in matrix market format): " << filename << "_U.mm" <<
