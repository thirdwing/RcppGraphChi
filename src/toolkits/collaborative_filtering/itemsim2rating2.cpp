...
bool is_item(vid_t v) {
    return v >= M;
}
bool is_user(vid_t v) {
    return v < M;
}
vec degrees;
/**
...
class adjlist_container {
  public:
    ...
    adjlist_container() {
        if (debug)
            Rcpp::Rcout<<"setting pivot st and end to " << 0 << std::endl;
    ...
    void clear() {
        ...
        adjs.clear();
        if (debug)
            Rcpp::Rcout<<"setting pivot st to " << pivot_en << std::endl;
        pivot_st = pivot_en;
    }
    ...
    double compute_ratings(graphchi_vertex<uint32_t, edge_data> &item, vid_t user_pivot, float user_item_edge_weight) {
        ...
        if (!get_val(pivot_edges.edges, item.id())) {
            if (debug)
                Rcpp::Rcout<<"Skipping item pivot pair since not connected!" << item.id() << std::endl;
            return 0;
        }

        int num_edges = item.num_edges();
        if (debug)
            Rcpp::Rcout<<"Found " << num_edges << " edges from item : " << item.id()-M+1 << std::endl;
        ...
        if (num_edges < min_allowed_intersection || nnz(pivot_edges.edges) < min_allowed_intersection) {
            if (debug)
                Rcpp::Rcout<<"skipping item pivot pair since < min_allowed_intersection" << std::endl;
            return 0;
        }
        ...
        for(int i=0; i < num_edges; i++) {
            ...
            if (other_item < M ) {
                if (debug)
                    Rcpp::Rcout<<"skipping an edge to item " << other_item << std::endl;
                continue;
            }
            ...
            if (debug)
                Rcpp::Rcout<<"Checking now edge number " << i << "  " << item.id()-M+1 << " -> " << other_item-M+1 << " weight: " <<
            ...
            if ((up && item.edge(i)->get_data().up_weight == 0) ||
                ...
                if (debug)
                    Rcpp::Rcout<<"skipping edge with wrong direction to " << other_item-M+1 << std::endl;
                continue;
            }

            if (get_val(pivot_edges.edges, other_item)) {
                if (debug)
                    Rcpp::Rcout<<"skipping edge to " << other_item << " because alrteady connected to pivot" << std::endl;
                continue;
            }
            ...
            if (weight == 0)
                Rcpp::Rcerr<<"Bug: found zero edge weight between: " << item.id()-M+input_file_offset << " -> " << other_item-M+input_file_offset <<std::endl;

            if (weight <= 1) {
                if (debug)
                    Rcpp::Rcout<<"skipping edge to " << item.id()-M+1 << " -> " << other_item-M+1 << " because of similarity is smaller or equal to one: " << weight << std::endl;
                continue;
            }
            ...
            if (debug) {
                Rcpp::Rcout<<"Adding weight: " << (((user_item_edge_weight-0.5)/0.5)* (weight- 1)) << " to item: " << other_item-M+1 << " for user: " << user_pivot+1<< " weight-1: " << weight-1<<std::endl;
                Rcpp::Rcout<<pivot_edges.ratings<<std::endl;
            }
        ...
        if (debug)
            Rcpp::Rcout<<"Finished user pivot " << user_pivot << std::endl;
        return 0;
    }
        ...
        */
            ...
            for (int i=0; i< v.num_edges(); i++) {
                ...
                if (item_pairs_compared % 1000000 == 0)
                    Rcpp::Rcout<< std::setw(10) << mytimer.current_time() << ")  " << std::setw(10) << item_pairs_compared << " pairs compared " << std::setw(10) << sum(written_pairs) << std::endl;
            }
        ...
        if (gcontext.iteration % 2 == 1) {
            ...
            for (int i=0; i< (int)adjcontainer->adjs.size(); i++) {
                ...
                FOR_ITERATOR(j, user.ratings) {
                    ...
                    if (degree_x <= 0)
                        Rcpp::Rcout<<"Item degree is 0: " << user_id << " -> " << item_id << std::endl;
                    assert(degree_x > 0);
                ...
                if (debug)
                    Rcpp::Rcout<<positions<<std::endl;
                assert(positions.size() > 0);
                positions.conservativeResize(std::min(nnz(user.ratings),(int)K));
                for (int j=0; j < positions.size(); j++) {

                    if (positions[j] >= (int)N) {
                        Rcpp::Rcout<<"bug: user rating " << user.ratings << " pos: " << positions << std::endl;
                        continue;
                    }
                    ...
                    if (debug)
                        Rcpp::Rcerr<<"Writing rating from user" << user.vid+1 << " to item: " << positions[j]-M+1 << std::endl;
                    assert(rc > 0);
...
int main(int argc, const char ** argv) {
    ...
    if (debug) for (int i=0; i< degrees.size(); i++)
            Rcpp::Rcout<<"degree of node: " << i << " is: " << degrees[i]<<std::endl;
    ...
    Rcpp::Rcout<<"Total item pairs compared: " << item_pairs_compared << " total written to file: " << sum(written_pairs) << std::endl;

    Rcpp::Rcout<<"Going to sort and merge output files " << std::endl;
