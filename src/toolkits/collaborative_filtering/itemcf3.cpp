...
class adjlist_container {
  ...
  public:
    ...
    void clear() {
        ...
        adjs.clear();
        if (debug)
            Rcpp::Rcout<<"setting pivot end to " << pivot_en << std::endl;
        pivot_st = pivot_en;
    }
    ...
    int load_edges_into_memory(graphchi_vertex<VertexDataType, EdgeDataType> &v) {
        ...
        if (num_edges < min_allowed_intersection) {
            if (debug)
                Rcpp::Rcerr<<"Skipping since num edges: " << num_edges << std::endl;
            return 0;
        }
...
int main(int argc, const char ** argv) {
    ...
    if (distance_metric != JACCARD_WEIGHT)
        Rcpp::Rcerr<<"--distance_metrix=XX should be one of:9= JACCARD_WEIGHT" << std::endl;
    ...
    Rcpp::Rcout<<"Created output files with the format: " << training << ".outXX, where XX is the output thread number" << std::endl;
