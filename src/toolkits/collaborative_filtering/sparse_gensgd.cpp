...
float get_value(char * pch, bool read_only) {
    ...
    if (std::isnan(ret) || std::isinf(ret))
        Rcpp::Rcerr<<"Failed to read value" << std::endl;
    return ret;
}
        ...
        if (token == fc.from_pos) {
            ...
            if (type != TRAINING && pos == (uint)-1) { //this feature was not observed on training, skip
                ...
                if (pch2 == NULL || pch2[0] == 0)
                    Rcpp::Rcerr<<"Error reading line " << i << " feature2 " << index << " [ " << linebuf_debug << " ] " << std::endl;
                skipped_features++;
                continue;
...
int convert_matrixmarket_N(std::string base_filename, bool square, feature_control & fc, int limit_rating = 0) {
    ...
    uint I, J;
    std::vector<uint> valarray;
    valarray.resize(FEATURE_WIDTH);
    std::vector<uint> positions;
    positions.resize(FEATURE_WIDTH);
    float val = 0.0f;
    ...
    for (size_t i=0; i<nz; i++) {
        ...
        if (nz > 1000000 && (i % 1000000) == 0)
            Rcpp::Rcout<< mytimer.current_time() << " Finished reading " << i << " lines " << std::endl;
        //calc stats
        L++;
        globalMean += val;
    ...
    assert(globalMean != 0);
    globalMean /= L;
    Rcpp::Rcout<<"Coputed global mean is: " << globalMean << std::endl;
...
void validation_rmse_N(
    ...
    if (f == NULL) {
        Rcpp::Rcout<<"Failed to open validation file: " << validation << " -  skipping."<<std::endl;
        return;
    }
    ...
    for (size_t i=0; i<nz; i++) {
        ...
        if (!read_line(f, validation, i, I, J, val, valarray, positions, index, VALIDATION, skipped_features))
            Rcpp::Rcerr<<"Failed to read line: " << i << " in file: " << validation << std::endl;
    ...
    Rcpp::Rcout<<"  Validation RMSE: " << std::setw(10) << dvalidation_rmse;
    if (calc_error)
        Rcpp::Rcout<<" Validation Err: " << std::setw(10) << ((double)errors/(double)(nz-skipped_nodes));
    ...
    if (calc_roc) {
        ...
        Rcpp::Rcout<<" Validation ROC: " << roc << std::endl;
    ...
    if (skipped_features > 0)
        Rcpp::Rcerr<<"Skipped " << skipped_features << " when reading from file. " << std::endl;
    if (skipped_nodes > 0)
        Rcpp::Rcerr<<"Skipped " << skipped_nodes << " when reading from file. " << std::endl;
}
...
void test_predictions_N(
    ...
    if (f == NULL) {
        Rcpp::Rcout<<"Failed to open test file " << test<< " skipping test predictions " << std::endl;
        return;
    }
    ...
    if (skipped_features > 0)
        Rcpp::Rcerr<<"Skipped " << skipped_features << " when reading from file. " << std::endl;
    if (skipped_nodes > 0)
        Rcpp::Rcout<<"Skipped node in test dataset: " << skipped_nodes << std::endl;
}
...
void output_gensgd_result(std::string filename) {
    ...
    Rcpp::Rcout << " GENSGD output files (in matrix market format): " << filename << "_U.mm" << ",  "<< filename <<  "_global_mean.mm, " << filename << "_U_bias.mm "  <<std::endl;
}
...
int main(int argc, const char ** argv) {
    ...
    if (has_header_titles && header_titles.size() == 0)
        Rcpp::Rcerr<<"Please delete temp files (using : \"rm -f " << training << ".*\") and run again" << std::endl;
