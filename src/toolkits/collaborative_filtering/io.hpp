...
FILE * open_file(const char * name, const char * mode, bool optional = false){
  ...
  if (f == NULL && !optional){
    perror("fopen failed");
    Rcpp::Rcerr <<" Failed to open file" << name << std::endl;
  }
  return f;
}
...
void read_matrix_market_banner_and_size(FILE * f, MM_typecode & matcode, uint & Me, uint & Ne, size_t & nz, const std::string & filename){
  ...
  if (mm_read_banner(f, &matcode) != 0)
    Rcpp::Rcerr << "Could not process Matrix Market banner. File: " << filename << std::endl;

  /*  This is how one can screen matrix types if their application */
  /*  only supports a subset of the Matrix Market data types.      */
  if (mm_is_complex(matcode) || !mm_is_sparse(matcode))
    Rcpp::Rcerr << "Sorry, this application does not support complex values and requires a sparse matrix." << std::endl;

  /* find out size of sparse matrix .... */
  if (mm_read_mtx_crd_size(f, &Me, &Ne, &nz) != 0) {
    Rcpp::Rcerr << "Failed reading matrix size: error" << std::endl;
  }
}
...
void detect_matrix_size(std::string filename, FILE *&f, uint &_MM, uint &_NN, size_t & nz, uint nodes = 0, size_t edges = 0, int type = TRAINING){
  ...
  if ((f = fopen(filename.c_str(), "r")) == NULL) {
    ...
    else Rcpp::Rcerr<<"Failed to open input file: " << filename << std::endl;
  }
...
void read_global_mean(std::string base_filename, int type){
  ...
  if (rc != 5)
    Rcpp::Rcerr<<"Failed to read global mean from file" << base_filename << ".gm" << std::endl;
  fclose(inf);
  if (type == TRAINING)
    Rcpp::Rcout << "Opened matrix size: " <<M << " x " << N << " edges: " << L << " Global mean is: " << globalMean << " time bins: " << K << " Now creating shards." << std::endl;
  else
    Rcpp::Rcout << "Opened VLIDATION matrix size: " <<Me << " x " << Ne << " edges: " << Le << " Global mean is: " << globalMean2 << " time bins: " << K << " Now creating shards." << std::endl;
}
...
void compute_matrix_size(size_t nz, int type){
  ...
  if (type == TRAINING)
    Rcpp::Rcout << "Starting to read matrix-market input. Matrix dimensions: "
      << M << " x " << N << ", non-zeros: " << L << std::endl;
  else
    Rcpp::Rcout << "Starting to read VALIDATION matrix-market input. Matrix dimensions: "
...
int convert_matrixmarket4(std::string base_filename, bool add_time_edges = false, bool square = false, int type = TRAINING, int matlab_time_offset = 1) {
  ...
  if (f == NULL){
    if (type == VALIDATION){
      Rcpp::Rcout<< "Did not find validation file: " << base_filename << std::endl;
      return -1;
    }
    else if (type == TRAINING)
      Rcpp::Rcerr<<"Failed to open training input file: " << base_filename << std::endl;
  }
 ...
 compute_matrix_size(nz, type); 
  ...
  bool active_edge = true;
    ...
    for (size_t i=0; i<nz; i++)
    {
      ...
      if (rc != 4)
        Rcpp::Rcerr<<"Error when reading input file - line " << i << std::endl;
      if (time < 0)
        Rcpp::Rcerr<<"Time (third columns) should be >= 0 " << std::endl;
      I-=input_file_offset;  /* adjust from 1-based to 0-based */
      J-=input_file_offset;
      if (I >= M)
        Rcpp::Rcerr<<"Row index larger than the matrix row size " << I << " > " << M << " in line: " << i << std::endl;
      if (J >= N)
        Rcpp::Rcerr<<"Col index larger than the matrix col size " << J << " > " << N << " in line; " << i << std::endl;
      K = std::max((int)time, (int)K);
      time -= matlab_time_offset;
      if (time < 0 && add_time_edges)
        Rcpp::Rcerr<<"Time bins should be >= " << matlab_time_offset << " in row " << i << std::endl;
    ...
    if (type == TRAINING){
      ...
      L += toadd;
      globalMean /= L;
      Rcpp::Rcout << "Global mean is: " << globalMean << " time bins: " << K << " . Now creating shards." << std::endl;
    }
    else {
      globalMean2 /= Le;
      Rcpp::Rcout << "Global mean is: " << globalMean2 << " time bins: " << K << " . Now creating shards." << std::endl;
    }
  ...
  Rcpp::Rcout << "Successfully finished sharding for " << base_filename<< std::endl;
...
int convert_matrixmarket_and_item_similarity(std::string base_filename, std::string similarity_file, int tokens_per_row, vec & degrees) {
  ...
  if (fsim == NULL || nz_sim == 0)
    Rcpp::Rcerr<<"Failed to open item similarity input file: " << similarity_file << std::endl;
  if (N_row != N || N_col != N)
    Rcpp::Rcerr<<"Wrong item similarity file matrix size: " << N_row <<" x " << N_col << "  Instead of " << N << " x " << N << std::endl;
  L=nz + nz_sim;
  ...
  unsigned int actual_edges = 0;
    ...
    for (size_t i=0; i<nz; i++){
      if (tokens_per_row == 3){
        ...
        if (rc != 3)
          Rcpp::Rcerr<<"Error when reading input file in line: " << i << std::endl;
        if (val == 0 && ! allow_zeros)
          Rcpp::Rcerr<<"Zero weight encountered at input file line: " << i << " . Run with --allow_zeros=1 to ignore zero weights." << std::endl;
      ...
      //Rcpp::Rcout<<"adding an edge: " <<I << " -> " << M+J << std::endl;
      actual_edges++;
    }

    Rcpp::Rcerr<<"Finished loading " << actual_edges << " ratings from file: " << base_filename << std::endl;

    for (size_t i=0; i<nz_sim; i++){
      if (tokens_per_row == 3){
        int rc = fscanf(fsim, "%u %u %lg\n", &I, &J, &val);
        if (rc != 3)
          Rcpp::Rcerr<<"Error when reading input file: " << similarity_file << " line: " << i << std::endl;
      }
      else if (tokens_per_row == 2){
      ...
      if (J >= N)
        Rcpp::Rcerr<<"Col index larger than the matrix col size " << J << " > " << N << " in line; " << i << std::endl;
      if (I == J)
        Rcpp::Rcerr<<"Item similarity to itself found for item " << I << " in line; " << i << std::endl;
      //Rcpp::Rcout<<"Adding an edge between "<<M+I<< " : " << M+J << "  " << (I<J)  << " " << val << std::endl; 
      sharderobj.preprocessing_add_edge(M+I, M+J, als_edge_type(I < J? val: 0, I>J? val: 0));
      actual_edges++;
    }

    L = actual_edges;
    Rcpp::Rcerr<<"Finished loading " << nz_sim << " ratings from file: " << similarity_file << std::endl;
    write_global_mean(base_filename, TRAINING);
    sharderobj.end_preprocessing();

    if (zero_entries)
      Rcpp::Rcout<<"Found " << zero_entries << " edges with zero weight!" << std::endl;
...
int convert_matrixmarket(std::string base_filename, size_t nodes = 0, size_t edges = 0, int tokens_per_row = 3, int type = TRAINING, int allow_square = true) {
  ...
  if (f == NULL){
    if (type == TRAINING){
      ...
      Rcpp::Rcout<<"Validation file: "  << base_filename << " is not found. " << std::endl;
      return -1;
    }
  }
  ...
  for (size_t i=0; i<nz; i++)
    {
      if (tokens_per_row == 3){
        ...
        if (rc != 3)
          Rcpp::Rcerr<<"Error when reading input file: " << i << std::endl;
        if (val == 0 && ! allow_zeros)
          Rcpp::Rcerr<<"Encountered zero edge [ " << I << " " <<J << " 0] in line: " << i << " . Run with --allow_zeros=1 to ignore zero weights." << std::endl;
        else if (val == 0){
           zero_entries++;
           continue;
        }
      ...
      if (I >= M)
        Rcpp::Rcerr<<"Row index larger than the matrix row size " << I+1 << " > " << M << " in line: " << i << std::endl;
      if (J >= N)
        Rcpp::Rcerr<<"Col index larger than the matrix col size " << J+1 << " > " << N << " in line; " << i << std::endl;
      if (minval != -1e100 && val < minval)
        Rcpp::Rcerr<<"Found illegal rating value: " << val << " where min value is: " << minval << std::endl;
      if (maxval != 1e100 && val > maxval)
        Rcpp::Rcerr<<"Found illegal rating value: " << val << " where max value is: " << maxval << std::endl;
    ...
    if (type == TRAINING){
      ...
      L += toadd;
      globalMean /= L;
      Rcpp::Rcout << "Global mean is: " << globalMean << " Now creating shards." << std::endl;
    }
    else {
      globalMean2 /= Le;
      Rcpp::Rcout << "Global mean is: " << globalMean2 << " Now creating shards." << std::endl;
    }
    write_global_mean(base_filename, type);
    sharderobj.end_preprocessing();

  if (zero_entries)
     Rcpp::Rcout<<"Found " << zero_entries << " zero edges!" << std::endl; 
  fclose(f);
    ...
    int type, bool optional_field, bool allow_zeros, int offset = 0)
{
  ...
  Rcpp::Rcout <<"Going to read matrix market vector from input file: " << filename << std::endl;
  ...
  if (mm_read_banner(f, &matcode) != 0)
    Rcpp::Rcerr << "Could not process Matrix Market banner." << std::endl;
  ...
  if (mm_is_complex(matcode) && mm_is_matrix(matcode) &&
    ...
    Rcpp::Rcerr << "sorry, this application does not support " << std::endl <<
      "Market Market type: " << mm_typecode_to_str(matcode) << std::endl;

  /* find out size of sparse matrix .... */
  if (mm_is_sparse(matcode)){
    if ((ret_code = mm_read_mtx_crd_size(f, &M, &N, &nz)) !=0)
      Rcpp::Rcerr << "failed to read matrix market cardinality size " << std::endl;
  }
  else {
    if ((ret_code = mm_read_mtx_array_size(f, &M, &N))!= 0)
      Rcpp::Rcerr << "failed to read matrix market vector size " << std::endl;
  ...
  for (i=0; i<nz; i++)
  {
    if (mm_is_sparse(matcode)){
      int rc = fscanf(f, "%u %u %lg\n", &row, &col, &val);
      if (rc != 3){
        Rcpp::Rcerr << "Failed reading input file: " << filename << "Problm at data row " << i << " (not including header and comment lines)" << std::endl;
      }
    ...
    assert(col == 0);
    if (val == 0 && !allow_zeros)
      Rcpp::Rcerr<<"Zero entries are not allowed in a sparse matrix market vector. Use --zero=true to avoid this error"<<std::endl;
...
vec load_matrix_market_vector(const std::string & filename,  bool optional_field, bool allow_zeros)
{
  ...
  fclose(f);
  Rcpp::Rcout<<"Succesfully read a vector of size: " << M << " [ " << nz << "]" << std::endl;
  return ret;
}
...
void save_matrix_market_format_vector(const std::string datafile, const vec & output, bool issparse, std::string comment)
{
  ...
  if (f == NULL)
    Rcpp::Rcerr<<"Failed to open file: " << datafile << " for writing. " << std::endl;
...
inline void write_output_vector(const std::string & datafile, const vec& output, bool issparse, std::string comment = ""){
  ...
  Rcpp::Rcout<<"Going to write output to file: " << datafile << " (vector of size: " << output.size() << ") " << std::endl;
...
void load_matrix_market_matrix(const std::string & filename, int offset, int D){
  ...
  if (rc != 0)
    Rcpp::Rcerr<<"Failed to load matrix market banner in file: " << filename << std::endl;
  ...
  if (D != (int)cols)
    Rcpp::Rcerr<<"Wrong matrix size detected, command line argument should be --D=" << D << " instead of : " << cols << std::endl;

  for (i=0; i<nnz; i++){
    if (mm_is_sparse(matcode)){
      rc = fscanf(f, "%u %u %lg\n", &I, &J, &val);
      if (rc != 3)
        Rcpp::Rcerr<<"Error reading input line " << i << std::endl;
      I--; J--;
      assert(I >= 0 && I < rows);
      assert(J >= 0 && J < cols);
      //set_val(a, I, J, val);
      ...
      rc = fscanf(f, "%lg", &val);
      if (rc != 1)
        Rcpp::Rcerr<<"Error reading nnz " << i << std::endl;
      I = i / cols;
      J = i % cols;
      latent_factors_inmem[I+offset].set_val(J, val);
    }
  }
  Rcpp::Rcout << "Factors from file: loaded matrix of size " << rows << " x " << cols << " from file: " << filename << " total of " << nnz << " entries. "<< i << std::endl;
  fclose(f);
}
