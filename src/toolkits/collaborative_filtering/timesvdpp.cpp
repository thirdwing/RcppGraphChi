...
float time_svdpp_predict(const time_svdpp_usr & usr,
    ...
    if (std::isnan(prediction))
        Rcpp::Rcerr<<"Got into numerical errors! Try to decrease --lrate, --gamma, --beta" <<std::endl;
...
void output_timesvdpp_result(std::string filename) {
    ...
    Rcpp::Rcout << " time-svd++ output files (in matrix market format): " << filename << "_U.mm" << ", " << filename + "_V.mm " << filename + "_T.mm, " << filename << " _global_mean.mm, " << filename << "_U_bias.mm " << filename << "_V_bias.mm " << std::endl;
}
