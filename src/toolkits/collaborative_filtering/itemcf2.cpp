...
class adjlist_container {
  ...
  public:
    ...
    adjlist_container() {
        if (debug)
            Rcpp::Rcout<<"setting pivot st and end to " << M << std::endl;
    ...
    double calc_distance(graphchi_vertex<uint32_t, float> &v, vid_t pivot, int distance_metric) {
        ...
        if (distance_metric == PEARSON) {
            if (debug) {
                Rcpp::Rcout<< pivot -M+1<<" Pivot edges: " <<pivot_edges.edges << std::endl;
                Rcpp::Rcout<< "Minusmean:   " << minus(pivot_edges.edges,mean) << std::endl;
                Rcpp::Rcout<< v.id() -M+1<<"Item edges:  " <<item_edges.edges << std::endl;
                Rcpp::Rcout<< "Minusmean:   " << minus(item_edges.edges, mean) << std::endl;
            }
            double dist = minus(pivot_edges.edges, mean).dot(minus(item_edges.edges, mean));
            if (debug)
                Rcpp::Rcout<<"dist " << pivot-M+1 << ":" << v.id()-M+1 << " " << dist << std::endl;
    ...
    void update(graphchi_vertex<VertexDataType, EdgeDataType> &v, graphchi_context &gcontext) {
        ...
        //at the first iteration compute the stddev of each item from the mean
            ...
            if (is_item(v.id())) {
                ...
                if (debug)
                    Rcpp::Rcout<<"item: " << v.id() - M+1 << " stddev: " << stddev[v.id() - M] << std::endl;
            }
        }
         ...
         */
            ...
            for (vid_t i=adjcontainer->pivot_st; i< adjcontainer->pivot_en; i++) {
                ...
                if (item_pairs_compared % 100000 == 0)
                    Rcpp::Rcout<< std::setw(10) << mytimer.current_time() << ")  " << std::setw(10) << item_pairs_compared << " pairs compared " << std::setw(10) <<sum(written_pairs) << " written. " << std::endl;
                if (debug)
            ...
            for (uint i=0; i< std::min(heap.size(), (size_t)K); i++) {
                ...
                if (rc <= 0) {
                    ...
                    Rcpp::Rcerr<<"Failed to write output to: file: " << training << omp_get_thread_num() << ".out" << std::endl;
                }
            }
...
int main(int argc, const char ** argv) {
    ...
    if (distance_metric != PEARSON && distance_metric != MANHATTEN && distance_metric != COSINE &&
        ...
        Rcpp::Rcerr<<"--distance_metrix=XX should be one of: 3=PEARSON, 4=COSINE, 5=CHEBYCHEV, 6=MANHATTEN, 7=TANIMOTO, 8=LOG_LIKELIHOOD, 9 = SLOPE_ONE" << std::endl;
    debug                    = get_option_int("debug", 0);
    parse_command_line_args();

    //if (distance_metric != JACKARD && distance_metric != AA && distance_metric != RA)
    //  Rcpp::Rcerr<<"Wrong distance metric. --distance_metric=XX, where XX should be either 0) JACKARD, 1) AA, 2) RA" << std::endl;

    mytimer.start();
    int nshards          = convert_matrixmarket<EdgeDataType>(training, 0, 0, 3, TRAINING, false);
    if (nshards != 1)
        Rcpp::Rcerr<<"This application currently supports only 1 shard" << std::endl;
    K                        = get_option_int("K", K);
    if (K <= 0)
        Rcpp::Rcerr<<"Please specify the number of ratings to generate for each user using the --K command" << std::endl;
    ...
    Rcpp::Rcout<<"Total item pairs compared: " << item_pairs_compared << " total written to file: " << sum(written_pairs) << " pairs with zero distance: " << zero_dist << std::endl;
    if (not_enough)
        Rcpp::Rcout<<"Items that did not have enough similar items: " << not_enough << std::endl;
    ...
    if (pmm == NULL)
        Rcpp::Rcerr<<"Failed to open " << training << ":info to file" << std::endl;
