...
void output_baseline_result(std::string filename) {
    ...
    Rcpp::Rcout << "Baseline output files (in matrix market format): " << filename << (algo == USER_MEAN ? ".baseline_user" : ".baseline_item") << std::endl;
}
...
int main(int argc, const char ** argv) {
    ...
    if (algorithm == "global_mean")
        ...
        algo = ITEM_MEAN;
    else Rcpp::Rcerr<<"Unsupported algorithm name. Should be --algorithm=XX where XX is one of [global_mean,user_mean,item_mean] for example --algorithm=global_mean" << std::endl;
