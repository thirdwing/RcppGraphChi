...
void output_als_result(std::string filename) {
    ...
    Rcpp::Rcout << "ALS output files (in matrix market format): " << filename << "_U.mm" <<
...
int main(int argc, const char ** argv) {
    ...
    if (user_sparsity < 0.5 || user_sparsity >= 1)
        Rcpp::Rcerr<<"Sparsity level should be [0.5,1). Please run again using --user_sparsity=XX in this range" << std::endl;

    if (movie_sparsity < 0.5 || movie_sparsity >= 1)
        Rcpp::Rcerr<<"Sparsity level should be [0.5,1). Please run again using --movie_sparsity=XX in this range" << std::endl;

    if (algorithm != SPARSE_USR_FACTOR && algorithm != SPARSE_BOTH_FACTORS && algorithm != SPARSE_ITM_FACTOR)
        Rcpp::Rcerr<<"Algorithm should be 1 for SPARSE_USR_FACTOR, 2 for SPARSE_ITM_FACTOR and 3 for SPARSE_BOTH_FACTORS" << std::endl;
